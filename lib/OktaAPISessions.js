 ...
 * @constructor
 */
function OktaAPISessions(apiToken, clientId, clientSecret, domain, preview)
{
    if(apiToken == undefined || domain == undefined) {
        throw new Error("OktaAPI requires an API token, domain");
    }
    this.domain = domain;
    if(preview == undefined) this.preview = false;
    else this.preview = preview;
    this.request = new NetworkAbstraction(apiToken, clientId, clientSecret, domain, preview);
}
